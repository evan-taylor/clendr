...
import { format, startOfDay, addDays, isSameDay, getHours, getMinutes, isToday, parseISO } from 'date-fns';
import { useCalendar } from '../CalendarContext';
import { getDaysInWeek, getHoursInDay, getPositionFromTime } from '../utils/dateUtils';
...
export default function WeekView() {
  ...
  const getEventsForDay = (day: Date): CalendarEvent[] => {
    return events.filter(event => {
      ...
      return isSameDay(eventStartDay, currentDay) && !event.is_all_day;
    });
  };
  ...
  const handleMouseUp = (day: Date) => {
    ...
    document.removeEventListener('mouseup', () => handleMouseUp(day));
  ...
  const renderAllDayEvents = () => {
    ...
    return (
      <div className="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
        ...
        <div className="col-span-6 grid grid-cols-7 py-1 min-h-[60px]">
          {weekDays.map((day, index) => {
            ...
            return (
              <div 
                key={index} 
                className={`border-r border-gray-200 dark:border-gray-700 h-full px-1 relative ${
                  index === 6 ? 'border-r-0' : ''
                } ${
      ...
      <div className="grid grid-cols-8 border-b border-gray-200 dark:border-gray-700">
        ...
        {weekDays.map((day, index) => (
          <div 
            key={index} 
            className={cn(
              "py-3 text-center border-r border-gray-200 dark:border-gray-700 flex flex-col items-center",
              isToday(day) ? "bg-blue-50 dark:bg-blue-900/10" : "",
              index === 6 ? "border-r-0" : ""
            )}
          >
            ...
            <div className={cn(
              "flex items-center justify-center w-8 h-8 mt-1 text-sm font-semibold rounded-full",
              isToday(day) 
              ...
              >
                ...
                {getEventsForDay(day).map(event => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className={`absolute left-0 right-1 px-1 py-0.5 text-xs truncate rounded border-l-4 hover:z-20
                        ${event.calendar?.color ? 
                        `border-[${event.calendar.color}] bg-${event.calendar.color}/10 text-${event.calendar.color}/90` : 
                    ...
                    >
                      ...
                      {eventStyle.height && parseFloat(eventStyle.height.toString()) > 3 && (
